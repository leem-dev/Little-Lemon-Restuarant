...
const Main = () =>  {
    ...
    const changeTime = (state, date) => {
        return {availNewTimes: fetchAPI(new Date())}
    }

    const initialState = {availNewTimes: fetchAPI(new Date())};
    const [state, dispatch] = useReducer(changeTime, initialState);
    ...
    return (
        <div>
            <Routes>
                ...
                <Route path="/booking" element={<BookingPage availNewTimes={state} dispatch={dispatch} submitForm={submitForm} />}/>
